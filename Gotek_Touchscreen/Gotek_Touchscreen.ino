...
// Embedded sample game (see ensureAmiCrushInstalled below). Regenerated by
// tools/pack-amicrush.py. Shipped uncompressed as a raw PROGMEM byte array
// and copied byte-for-byte to SD via memcpy_P — arduino-esp32 declares
// uncompress() via the ROM headers but doesn't link a miniz implementation,
// so the classic zlib route was abandoned in favour of a raw copy (adds
// ~880 KB to the firmware bin, comfortable in the ~3.1 MB APP slot on
// default_8MB). Cover art also ships as a plain JPEG blob and is written
// to SD verbatim.
...
#define FW_INTERNAL "release.043"
...
// ============================================================================
// AmiCrush sample game — install / repair on any boot (not just fresh SD)
// ============================================================================
//
// Copies the embedded AmiCrush 1.0 ADF + JPEG cover + NFO into
// /ADF/AmiCrush/ when they aren't already there OR are demonstrably broken.
// Runs on EVERY boot so pre-v0.11.2 SD cards (which have a CONFIG.TXT and
// therefore skip firstBootScaffold) still receive AmiCrush after the
// firmware upgrade.
//
// Per-file guards (fixes v0.11.5's blind exists() check):
//  * ADF/JPG — exists() AND size >= sanity floor (100 KB / 1 KB). A
//    truncated write from an interrupted install boot would otherwise
//    leave a 0-byte file that fools next boot's exists() into skipping
//    the reinstall forever.
//  * NFO — exists() is fine (a few hundred bytes of text; rewriting is
//    cheap but pointless).
//
// If the user deletes any of the three, next boot restores just that
// file — the other two aren't touched. An explicit "keep it deleted"
// flag can land later if this becomes annoying.

// True if `path` exists AND is at least `minSize` bytes. Zero-byte /
// truncated files fail the size check so the caller can rewrite them.
// Preserves user-supplied replacement ADFs (any legit Amiga image is
// well above the 100 KB floor).
static bool _amicrushHasValidFile(const char *path, size_t minSize) {
...
// Copy a PROGMEM byte array to SD and verify the final size. On any
// write short-count OR a mismatched post-write size the destination is
// removed so the next boot's guard rewrites cleanly. Uses a 4 KB heap
// buffer instead of a stack one — loopTask has ~8 KB stack budget on
// arduino-esp32 3.x and this runs alongside other setup work.
static bool _amicrushWriteProgmemFile(const char *path,
  ...
  while (written < len) {
    ...
    memcpy_P(buf, data + written, chunk);
    size_t got = f.write(buf, chunk);
  ...
  f.close();
  free(buf);
  if (!ok) { SD_MMC.remove(path); return false; }
  File check = SD_MMC.open(path, "r");
  if (!check) return false;
  size_t sz = check.size();
  check.close();
  if (sz != len) { SD_MMC.remove(path); return false; }
  return true;
}
...
void ensureAmiCrushInstalled() {
  ...
  if (!SD_MMC.exists("/ADF/AmiCrush")) SD_MMC.mkdir("/ADF/AmiCrush");

  Serial.println(String("AmiCrush install: needADF=") + needADF +
                 " needJPG=" + needJPG + " needNFO=" + needNFO);
  sdLog(String("AmiCrush: install begin needADF=") + needADF +
        " needJPG=" + needJPG + " needNFO=" + needNFO);

  if (needADF) {
    if (_amicrushWriteProgmemFile("/ADF/AmiCrush/AmiCrush.adf",
                                  amicrush_adf, amicrush_adf_len)) {
      Serial.println("  Wrote AmiCrush.adf (" +
                     String((uint32_t)amicrush_adf_len) + " bytes)");
      sdLog("AmiCrush: ADF ok (" + String((uint32_t)amicrush_adf_len) + " B)");
    } else {
      Serial.println("  FAILED: AmiCrush.adf");
      sdLog("AmiCrush: ADF write FAILED");
    }
  }

  if (needJPG) {
    if (_amicrushWriteProgmemFile("/ADF/AmiCrush/AmiCrush.jpg",
                                  amicrush_cover_jpg, amicrush_cover_jpg_len)) {
      Serial.println("  Wrote AmiCrush.jpg (" +
                     String((uint32_t)amicrush_cover_jpg_len) + " bytes)");
      sdLog("AmiCrush: JPG ok (" + String((uint32_t)amicrush_cover_jpg_len) + " B)");
    } else {
      Serial.println("  FAILED: AmiCrush.jpg");
      sdLog("AmiCrush: JPG write FAILED");
    }
  }
...
//
// Fallback denominator: WebDAV servers that don't advertise Content-Length
// (chunked encoding, or dynamic content) now pass total=0 from
// webdav_client.h — treated as a standard ~1.44 MB Amiga disk so the bar
// still moves. Overshoots real Amiga floppy size (901 120 B) so pct just
// caps at 99 % if the ADF is slightly larger — better than the 0→100 %
// jump this callback was written to fix.
static void _davDownloadProgressToLoadingScreen(size_t rec, size_t total) {
  const size_t FALLBACK = 1500u * 1024u;
