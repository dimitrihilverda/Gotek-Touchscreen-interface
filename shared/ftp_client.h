...
/*
  Gotek — Lightweight FTP Client (shared across devices)
...
public:
  ...
  bool connect() {
    ...
    _log("FTP: connecting to " + cfg_ftp_host + ":" + String(cfg_ftp_port));
    ...
    _connected = true;
    _log("FTP: logged in as " + cfg_ftp_user);
    return true;
  }
  ...
  bool listDir(const String &path, std::vector<FTPFileEntry> &entries) {
    ...
    _log("FTP: listed " + String(entries.size()) + " entries in " + fullPath);
    return true;
  }
  ...
  long downloadFile(const String &remotePath, const String &localPath) {
    ...
    _log("FTP: downloaded " + fullRemote + " -> " + localPath + " (" + String(totalBytes) + " bytes)");
    return totalBytes;
  }
  ...
  // Stream a file from FTP directly into a memory buffer (no SD card needed)
  // Returns bytes read, or -1 on error
  long streamToBuffer(const String &remotePath, uint8_t *destBuf, size_t maxLen) {
    _lastError = "";
    ...
    while (data.connected() || data.available()) {
      ...
      size_t remaining = maxLen - totalBytes;
      if (remaining == 0) break;  // buffer full
      size_t toRead = (avail > remaining) ? remaining : avail;
      size_t bytesRead = data.read(&destBuf[totalBytes], toRead);
      if (bytesRead > 0) {
        totalBytes += bytesRead;
        timeout = millis();
      }
    ...
    data.stop();
    _readResponse();  // Read 226 transfer complete
    ...
    _log("FTP: streamed " + fullRemote + " (" + String(totalBytes) + " bytes to buffer)");
    return totalBytes;
  }
...
private:
  ...
  // Logging — routes to sdLog() on SD-equipped devices, Serial.println() otherwise
    ...
    while (pos < (int)listing.length()) {
      ...
      for (int i = 0; i < (int)line.length(); i++) {
        ...
        if (inSpace && !isSpace) {
          fieldCount++;
          fieldIdx++;
          if (fieldIdx == 5) sizeField = i;
          if (fieldIdx >= 9) { nameStart = i; break; }
        }
        inSpace = isSpace;
      }
